...
* Minor changes made for Trinitycore, + Core additions
*/
#include "ScriptMgr.h"
#include "Player.h"
...
public:
    ...
    void RollPossibleEnchant(Player* player, Item* item)
    {
        ...
        if (slotRand[2] != -1)
            chathandle.PSendSysMessage("|cffFF0000 %s |rgot|cffFF0000 3 |rrandom enchantments!", item->GetTemplate()->Name1.c_str());
        else if (slotRand[1] != -1)
            chathandle.PSendSysMessage("|cffFF0000 %s |rgot|cffFF0000 2 |rrandom enchantments!", item->GetTemplate()->Name1.c_str());
        else if (slotRand[0] != -1)
            chathandle.PSendSysMessage("|cffFF0000 %s |rgot|cffFF0000 1 |rrandom enchantment!", item->GetTemplate()->Name1.c_str());
    }
