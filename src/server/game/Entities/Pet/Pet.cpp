...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    {
        //DWrath, checks entry id of each warlock summon and compares, if all are false, set hunter, else its warlock
        int cinfoen = cinfo->Entry;
        if (cinfo->Entry != 416 && cinfoen != 1860 && cinfoen != 1863 && cinfoen != 417 && cinfoen != 17252 && cinfoen != 19668)
        {
            petType = HUNTER_PET;
            m_unitTypeMask |= UNIT_MASK_HUNTER_PET;
        }else
        ...
        //DWrath edit, lol get dunked. Swapped check for a player instead of the class
        /*else if (GetOwner()->GetTypeId() == TYPEID_PLAYER) //GetClass() == CLASS_HUNTER)
        {
            petType = HUNTER_PET;
            m_unitTypeMask |= UNIT_MASK_HUNTER_PET;
        }
