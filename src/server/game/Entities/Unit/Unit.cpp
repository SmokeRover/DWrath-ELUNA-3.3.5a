...
/*static*/ uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    //DWrath edit, removed  playerpowertype == rage check, replaced with gettypeid instead
    if (attacker && cleanDamage && damagetype == DIRECT_DAMAGE && attacker != victim /*GetPowerType() == POWER_RAGE*/) {
    ...
    if (!damage)
    {
        ...
        if (cleanDamage && cleanDamage->absorbed_damage /*&& victim->GetPowerType() == POWER_RAGE*/) // DWrath, changed from get power type rage to gettypeid  player
    ...
    else
    {
        ...
        if (attacker != victim /*&& victim->GetPowerType() == POWER_RAGE*/) // DWrath, changed from getpowertype rage to gettypeid player
        {
...
void Unit::SetPowerType(Powers new_powertype, bool sendUpdate/* = true*/)
{
    ...
    {
        //DWrath, Added setpower rage to mana and energy
...
void Unit::UpdateDisplayPower()
{
    ...
    {
        //DWrath EDIT, removed power display changers, should work better with the yoinked addon
        case FORM_GHOUL:
        case FORM_CAT:
        ...
        default:
        {
            ...
            else if (GetTypeId() == TYPEID_UNIT)
            {
                ...
                {
                    //DWrath edit, super verbose way of forcing a hunter pet with focus without directly saying hunter :)
