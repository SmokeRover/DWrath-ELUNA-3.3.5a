...
LfgCompatibility LFGQueue::CheckCompatibility(GuidList check)
{
    ...
    if (!sLFGMgr->IsSoloLFG() && numPlayers != MAXGROUPSIZE) //solo lfg         // MICRO LFG     (check.size() == 1 && numPlayers != MAXGROUPSIZE)
    {
    ...
    // Enough players?
    if (!sLFGMgr->IsSoloLFG() && numPlayers != MAXGROUPSIZE) //solo lfg         // MICRO LFG     (numPlayers != MAXGROUPSIZE)
    {
